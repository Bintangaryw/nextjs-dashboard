...
export default async function LatestInvoices({ latestInvoices }: { latestInvoices: LatestInvoice[] }) {
    return (
        <div className="flex w-full flex-col md:col-span-4">
            <h2 className={`${lusitanaFont.className} mb-4 text-xl md:text-2xl`}>Latest Invoices</h2>
                    ...
                    {latestInvoices.map((invoice, i) => {
                        return (
                            ...
                            >
                                ...
                                <p className={`${lusitanaFont.className} truncate text-sm font-medium md:text-base`}>{invoice.amount}</p>
                            </div>
                        );
                    })}
                </div>
